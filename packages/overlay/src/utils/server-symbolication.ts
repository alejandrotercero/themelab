...
import {
  isServerComponentUrl,
  devirtualizeServerUrl,
...
function getNextBasePath(): string {
  if (cachedNextBasePath !== undefined) {
...
async function symbolicateServerFrames(
  frames: StackFrame[]
): Promise<StackFrame[]> {
  ...
  for (let frameIndex = 0; frameIndex < frames.length; frameIndex += 1) {
    ...
    if (!frame.isServer || !frame.fileName) {
      continue;
    }
  ...
  try {
    ...
    for (
      ...
      if (!resolved?.file || resolved.ignored) {
        continue;
      }
...
function extractServerFramesFromDebugStack(
  rootFiber: Fiber
  ...
  traverseFiber(
    ...
    (currentFiber) => {
      ...
      for (const frame of parseStack(ownerStack)) {
        ...
        if (!isServerComponentUrl(frame.fileName)) {
          continue;
        }
        if (serverFramesByName.has(frame.functionName)) {
          continue;
        }
...
function enrichServerFrameLocations(
  rootFiber: Fiber,
  frames: StackFrame[]
): StackFrame[] {
  ...
  if (!hasUnresolvedServerFrames) {
    return frames;
  }

  const serverFramesByName = extractServerFramesFromDebugStack(rootFiber);
  if (serverFramesByName.size === 0) {
    return frames;
  }

  return frames.map((frame) => {
    if (!frame.isServer || frame.fileName || !frame.functionName) {
      return frame;
    }
...
export async function getResolvedOwnerStack(
  ...
  if (!frames || frames.length === 0 || !isNextProjectRuntime()) {
    return frames;
  }
