...
import { HR_SLOTS } from "./types"
import type { HrSlot, HrTheme } from "./types"
...
export function parseHrSvg(svg: string): HrTheme {
  ...
  if (doc.querySelector("parsererror")) {
    throw new Error(
      "Could not parse SVG — is this a Hundred Rabbits theme file?"
    )
  }
  ...
  // oxlint-disable-next-line eslint/prefer-named-capture-group -- named groups compile to TS1503 under this project's ES2017 target
  ...
  if (Object.keys(slots).length === 0) {
    throw new Error(
      "No Hundred Rabbits color slots found (expected ids like background, f_high, b_inv)."
    )
  }
