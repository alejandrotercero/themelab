  ...
  return (
    <DotBackground className="min-h-svh">
      <main className="min-h-svh">
        ...
        <article className="max-w-3xl container mx-auto w-full px-[5%] py-16 md:py-24">
          <Logo className="mx-auto mb-4 max-w-1/2" />
          <header className="mb-12 md:mb-16">
            <p className="mb-3 font-heading text-sm tracking-wider text-primary uppercase">
              How it works
            </p>
            ...
            <p className="mt-5 text-lg leading-relaxed text-muted-foreground">
              ...
              server you already run and gives you a visual editing layer on top
              of your real app — every change you confirm is written straight
              back into your source files. No prompts to write, no files to hunt
              through, and no AI in the loop unless you ask for it.
            </p>
          </header>
          ...
          <div className="space-y-12 md:space-y-16">
            <Section title="The core idea">
              <p>
                ...
                right line, make the edit, switch back, wait for reload.
                ThemeLab collapses that loop. Because it proxies your dev server
                and injects an overlay, the page in your browser is your app —
                but now every element is selectable and editable. What you see
                is literally what you are editing.
              </p>
              <p>
                Nothing about your project changes to make this work. There is
                no plugin to add to your bundler, no provider to wrap your app
                in, and no build step. ThemeLab reads the React tree at runtime
                to map pixels back to components, and writes edits back as
                ordinary code changes you can review in git like any other diff.
              </p>
            </Section>
            ...
            <Section title="Deterministic by default">
              <p>
                Every edit ThemeLab applies is a precise, rule-based transform
                of your code — the same input always produces the same output.
                That means changes are predictable and reviewable: a class added
                here, a token updated there, a node moved in a list. There is no
                model guessing what you meant and no risk of an edit quietly
                rewriting code you did not touch.
              </p>
            </Section>

            <Section title="AI assist — optional, and only a locator">
              <p>
                A few cases are genuinely ambiguous in source: an element
                rendered by a .map(), a reused component instance, conditional
                or state-dependent rendering, or a component that outputs a
                different host tag than its name suggests (a Link that renders
                an a). For those, you can turn on an AI locator. It reads your
                source to find the exact node to edit — and then hands control
                back. The change itself is still applied by the same
                deterministic AST transform. The AI only locates; it never
                writes your code.
              </p>
              <p>
                It is off by default and runs only when you configure an
                ANTHROPIC_API_KEY. When an edit would affect more than the
                selected element — a .map() template or a shared component — it
                asks you to confirm first, and it caches what it finds so
                repeated tweaks stay instant. If it truly cannot find the
                element, it tells you why rather than guessing.
              </p>
            </Section>
            ...
            <Section title="It talks to your coding agent too">
              <p>
                ...
                what you are doing in the overlay. Click a component in the
                browser and your agent knows the exact file and line — no more
                &ldquo;which Button did you mean?&rdquo;. It can ask for the
                current selection, the resolved theme tokens, the Tailwind token
                map, or where a component lives in the source.
              </p>
            </Section>
