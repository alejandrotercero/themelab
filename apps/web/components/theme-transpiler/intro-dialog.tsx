...
export function IntroDialog({ open, onOpenChange }: IntroDialogProps) {
  return (
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent
        className="tl-overlay flex max-w-lg flex-col gap-4 p-6"
      ...
      >
        ...
        <div className="flex flex-col gap-3 text-sm leading-relaxed text-[var(--ov-text-dim)]">
          <p>
            <span className="text-[var(--ov-text)]">
              Hundred Rabbits (100r)
            </span>{" "}
            is a two-person studio that builds tiny, portable creative tools.
            Their theme format is dead simple: a single SVG that encodes just{" "}
            <span className="text-[var(--ov-text)]">9 colors</span> — a
            background, four foregrounds, and four backgrounds.
          </p>
          <p>
            That makes themes tiny, human-readable, and drag-and-drop portable
            across their whole ecosystem (Orca, Left, Dotgrid…). It&apos;s
            constraint-driven design — nine colors, no bloat.
          </p>
          <p>
            ...
            — 31 tokens, light and dark — using OKLCH interpolation. Drop an SVG
            and go.
          </p>
        </div>
