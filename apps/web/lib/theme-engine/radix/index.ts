...
export function radixThemeStyles({ light, dark }: RadixInputs): ThemeStyles {
  const lightOut = generateRadixColors({ appearance: "light", accent: light.accent, gray: light.gray, background: light.bg });
  const darkOut = generateRadixColors({ appearance: "dark", accent: dark.accent, gray: dark.gray, background: dark.bg });
  return {
...
export function radixScales(args: { accent: string; gray: string; background: string; appearance: Appearance }): {
  primary: Scale;
  neutral: Scale;
} {
  ...
  const toScale = (hexes: string[]): Scale => hexes.map((value, i) => ({ stop: i + 1, value: oklch(value) }));
