...
export function useThemeEditor() {
  ...
  const theme = useMemo(
    () => resolve(state.base, state.overrides),
  ...
  useEffect(() => {
    ...
    applyVars(
      root,
      { ...activeVars, ...ovSkinVars(activeVars) },
      { dark: state.mode === "dark", radius: state.radius }
    )
  ...
  return {
    ...
    setSwatches: (swatches: Swatch[]) =>
      dispatch({ type: "swatches", swatches }),
    setSavedId: (savedId: string | null) =>
      dispatch({ type: "savedId", savedId }),
    setApplyToSite,
  }
}
