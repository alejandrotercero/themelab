...
const PANEL_H = 380;
const CHECKER =
  "repeating-conic-gradient(#3a3a44 0% 25%, #555 0% 50%) 0 0 / 8px 8px";
...
function buildEntries(): Map<string, Entry[]> {
  ...
  for (const [family, shades] of Object.entries(TAILWIND_PALETTE)) {
    const list: Entry[] = [];
    for (const [shade, raw] of Object.entries(
      shades as Record<string, string>
    )) {
      const token = shade === "DEFAULT" ? family : `${family}-${shade}`;
      const label =
        shade === "DEFAULT" ? family[0].toUpperCase() + family.slice(1) : token;
...
function styleScrollbar(el: HTMLElement): void {
  ...
  (
    el.style as CSSStyleDeclaration & { scrollbarColor?: string }
  ).scrollbarColor = `${PANEL.border} transparent`;
}
...
export function openTailwindPalette(opts: TailwindPaletteOptions): void {
  ...
  const left = Math.max(
    8,
    Math.min(opts.anchorRect.right - PANEL_W, window.innerWidth - PANEL_W - 8)
  );
  ...
  toggle.append(listBtn);
  toggle.append(gridBtn);
  header.append(title);
  ...
  // --- Lifecycle ---
  // The abort controller detaches the document-level listeners registered
  // below, so close() doesn't need to reference the handlers directly.
  ...
  function filtered(): [string, Entry[]][] {
    ...
    for (const [family, list] of ALL_GROUPS) {
      ...
      const hits = list.filter((e) => e.token.toLowerCase().includes(q));
      if (hits.length) {
        out.push([family, hits]);
      }
    }
    return out;
  }
  ...
  function listRow(e: Entry): HTMLElement {
    ...
    row.addEventListener("mouseleave", () => {
      row.style.background = selected ? PANEL.surface : "transparent";
    });
    row.addEventListener("click", () => {
      opts.onPick(e.token, e.css);
      close();
    });
    return row;
  }
  ...
  function handleViewClick(kind: "list" | "grid"): void {
    view = kind;
    syncToggle();
    render();
  }
  listBtn.addEventListener("click", () => handleViewClick("list"));
  gridBtn.addEventListener("click", () => handleViewClick("grid"));
  ...
  function onDocMouseDown(ev: MouseEvent): void {
    ...
    if (opts.anchorEl && path.includes(opts.anchorEl)) {
      return;
    }
    close();
  }
