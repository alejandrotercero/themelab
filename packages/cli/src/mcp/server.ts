...
function buildMcpServer(deps: McpServerDeps): McpServer {
  ...
  server.registerTool(
    ...
    () => {
      if (!deps.isOverlayConnected()) {
        return textResult(NOT_CONNECTED);
      }
      const sel = deps.getSelection();
      if (!sel) {
        return textResult(
          "Nothing is selected in the overlay right now. Ask the user to click a component."
        );
      }
      return jsonResult({
  ...
  server.registerTool(
    ...
    () => {
      ...
      if (!t) {
        return textResult("No theme has been resolved for this project.");
      }
      return jsonResult(t);
    }
  );
  ...
  server.registerTool(
    ...
    () => {
      ...
      if (!tokens) {
        return textResult(
          "No Tailwind tokens have been resolved for this project."
        );
      }
  ...
  server.registerTool(
    "find_component",
    {
      ...
      inputSchema: {
        componentName: z
          .string()
          .describe('The component name to locate, e.g. "Button".'),
      },
    },
    async ({ componentName }) => {
      ...
      if (!filePath) {
        return textResult(
          `No source file found for component "${componentName}".`
        );
      }
...
export function createMcpHttpServer(
  ...
  async function handle(
    ...
    try {
      ...
      logger.warn("[ThemeLab] MCP request failed:", error);
      if (!res.headersSent) {
