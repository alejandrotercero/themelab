...
function Swatch({ label, value, onChange }: { label: string; value: string; onChange: (hex: string) => void }) {
  return (
    <div className="flex items-center gap-1.5">
      <SwatchPopover value={value} onChange={onChange} title={`${label}: ${value}`} className="size-[18px]" />
      <span className="text-[11px] text-[var(--ov-text-dim)]">{label}</span>
    </div>
  );
}

export function PaletteInput({ primary, neutral, onPrimary, onNeutral, onGenerate }: PaletteInputProps) {
  return (
