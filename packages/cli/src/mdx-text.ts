...
function isExactStart(node: MdxNode, line: number, col: number): boolean {
  ...
  return (
    start.line === line && (start.column === col || start.column === col + 1)
  );
}

function containsSourcePosition(
  node: MdxNode,
  line: number,
  col: number
): boolean {
  ...
  const startsBefore =
    line > start.line ||
    (line === start.line &&
      (targetColumn >= start.column || col >= start.column));
  const endsAfter =
    line < end.line ||
    (line === end.line && (targetColumn <= end.column || col <= end.column));
...
function resolveEditRange(
  ...
  if (diff.oldSubstring) {
    const hintedStart =
      renderedText.slice(
        diff.prefixLen,
        diff.prefixLen + diff.oldSubstring.length
      ) === diff.oldSubstring
        ? diff.prefixLen
  ...
  const insertionStart =
    cursorOffset === undefined
      ...
      : Math.max(
          0,
          Math.min(renderedText.length, cursorOffset - diff.newSubstring.length)
        );
  return {
...
function mapRenderedOffsetToRawIndex(
  ...
  if (segment.renderedChars.length === 0) {
    return 0;
  }
  if (renderedOffset <= 0) {
  ...
  if (renderedOffset >= segment.renderedChars.length) {
    const lastChar = segment.renderedChars.at(-1);
    return lastChar ? lastChar.rawEnd : 0;
  }
...
function applyInsertion(
  ...
  let previous: LeafSegment | undefined;
...
function applyEditToContainer(
  ...
  const firstRawStart = mapRenderedOffsetToRawIndex(
    first,
    firstLocalStart,
    "start"
  );
...
function buildLeafSegment(
  ...
  let renderedText = "";
  let rawIndex = 0;
  let { previousEndsWithWhitespace } = state;
...
function buildContainerCandidate(
  node: MdxNode,
  depth: number
...
function selectBestContainer(
  ...
  const exactTextMatches = candidates.filter(
    (candidate) =>
      normalizeTextForMatch(candidate.renderedText) === normalizedOriginal
  );
  ...
  if (exactTextMatches.length > 0) {
    pool = exactTextMatches;
  } else if (partialTextMatches.length > 0) {
    pool = partialTextMatches;
  }
...
function pruneEmptyNodes(node: MdxNode): void {
  ...
  for (const child of node.children) {
    ...
    if (
      ...
      PRUNABLE_EMPTY_NODE_TYPES.has(child.type)
    ) {
      continue;
    }
...
function serializeContainer(node: MdxNode): string {
  ...
  const serialized = String(
    mdxProcessor.stringify(
      node as unknown as ReturnType<typeof mdxProcessor.parse>
    )
  );
...
export function applyMdxTextEdit(
  ...
  const editRange = resolveEditRange(
