// packages/overlay/src/utils/parse-package-name.ts
//
// Ported from react-grab (packages/react-grab/src/utils/parse-package-name.ts).
// Extracts an npm package name from a frame's fileName when it points at a
// dependency, returning null for first-party app source. Used by
// classify-source-path.ts to decide app vs package origin.
//
// This is a positive classifier (recognise dependency shapes) rather than a
// denylist, so it handles cases ThemeLab's old `isLibraryPath` denylist missed:
//   - Vite optimized-deps flattening (`@radix-ui_react-dialog.js`)
//   - sourcemapped scoped deps with no node_modules marker (`../@acme/ui/...`)
//   - CDN URLs (esm.sh / unpkg style `@scope/name@version/...`)
// while keeping monorepo workspaces (ui, shared, app, …) classified as app.
...
const NAME_AT_VERSION_PATTERN = /^(?<name>.+?)@v?\d/;
...
const readViteOptimizedDepPackage = (afterMarker: string): string | null => {
  const [firstSegment] = splitPathSegments(afterMarker);
  if (!firstSegment) {
    return null;
  }
  const stem = firstSegment.replace(FILE_EXTENSION_PATTERN, "");
  if (VITE_INTERNAL_CHUNK_PATTERN.test(stem)) {
    return null;
  }
  if (!stem.startsWith("@")) {
    return stem;
...
const extractAfterLastMarker = (
  ...
  return parts.length > 1 ? read(parts.at(-1) as string) : null;
};

const extractNameAtVersion = (segment: string | undefined): string | null =>
  segment?.match(NAME_AT_VERSION_PATTERN)?.groups?.name ?? null;
...
const parseScopedPackageSourceName = (fileName: string): string | null => {
  ...
  const [scope, packageName, ...innerPathSegments] =
