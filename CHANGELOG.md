...
- **AI locator (optional, off by default)** — when deterministic resolution can't pin the selected element — a `.map()` template, a reused component instance, conditional/state-dependent rendering, a component that renders a different host tag (e.g. `<Link>` → `<a>`), or a computed value (`{count}`) — an opt-in AI reads the actual source to find the exact node. It only *locates*; the edit is still applied deterministically. Direct/conditional matches apply automatically; `map-template`/`instance` edits ask to confirm first. A "Locating with AI…" → "Found it" indicator shows while it works, resolutions are cached per element, and surrounding labels are used to anchor computed-value elements. If it genuinely can't find the element, it says *why* instead of guessing. Requires `ANTHROPIC_API_KEY`; with no key, behavior is unchanged.
- **Confirm with AI (⚡)** — a button beside Confirm that resolves every staged change via the AI locator up front, instead of trying deterministic first — useful when deterministic resolution lands on the wrong element.
- **AI fallback for move up/down + list reordering** — reordering an element also gets the AI locator. Reordering an item rendered by a `.map()` now swaps the matching element in the **source data array** (with a confirm), rather than failing or moving the wrong node.
- **AI settings** — a gear panel to store an API key, a custom endpoint (base URL), and a model. Persisted locally (`~/.config/themelab/config.json`, key never sent back to the overlay); environment variables override the stored values.
