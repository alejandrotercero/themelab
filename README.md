...
- **Smart retry:** if the first (fast, cheap) model fails or gives up, ThemeLab automatically retries with a stronger model ("Looking harder…"), giving it a bigger exploration budget and a note of what the first attempt already tried. On by default when AI is enabled — toggle it in the gear panel, or set `THEMELAB_AI_ESCALATION=0` / `THEMELAB_AI_MODEL_ESCALATED=<model>` to control it from the environment.
