...
export function detect(cwd?: string): DetectionResult {
  ...
  if (!fs.existsSync(pkgJsonPath)) {
    throw new Error(
      "No package.json found. Run themelab from your project root."
    );
  }
  ...
  if (!allDeps["react"]) {
    throw new Error(
      "React not found in dependencies. ThemeLab requires a React project."
    );
  }
  ...
  if (process.env.NODE_ENV === "production") {
    throw new Error(
      "ThemeLab requires development mode. React Fiber debug info is not available in production builds."
    );
  }
  ...
  throw new Error(
    "Could not detect framework. ThemeLab supports Next.js, Vite, and Create React App."
  );
}
...
export async function healthCheck(
  ...
  throw new Error(
    `No dev server found on ${host}:${port}. Start your dev server first, then run themelab.`
  );
}
