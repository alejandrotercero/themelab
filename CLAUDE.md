# CLAUDE.md — ThemeLab Project

## Project Overview

ThemeLab (npm: `themelab`) is a CLI tool that overlays on running React dev servers, enabling visual component selection, drag-to-reorder (writes to source JSX), and a Figma-style visual canvas with drawing, color, text, move, lasso tools. Built as a pnpm monorepo.
...
- `apps/web/` — Next.js theme studio (`/100r` HR→shadcn transpiler, `/create` palette→theme + scales). See `apps/web/CLAUDE.md`.
...
- All UI lives in Shadow DOM (`#themelab-root`) for style isolation
