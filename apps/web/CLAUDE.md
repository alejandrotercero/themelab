...
- **Kibo color picker** (`components/kibo-ui/color-picker`): its controlled `value` is buggy, so we drive it uncontrolled via `defaultValue` (re-seeded on each open). Patched: the selection thumb derives its position from the live saturation/lightness every render (drag, eyedropper, and typed values all move it — replacing Kibo's mount-only hack); the hex field is **editable** (`HexInput` — type/paste a `#rrggbb`); and the rgb/hsl modes are **editable numeric channels** (`ChannelInput`). The dead alpha **%** field was removed (tokens are opaque — alpha is ignored), which is also what unclipped the multi-field modes; css mode stays a read-only `rgb(...)` string.
