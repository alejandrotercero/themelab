...
import { parseHrSvg, PRESETS } from "@/lib/theme-engine"
import type { HrTheme } from "@/lib/theme-engine"
...
export function HrInput({ onLoad, onError }: HrInputProps) {
  ...
  const handleSvg = (svg: string, name: string) => {
    try {
      ...
      onError(error instanceof Error ? error.message : "Failed to parse theme.")
    }
  }
  ...
  const loadPreset = (id: string) => {
    ...
    if (preset) {
      handleSvg(preset.svg, preset.name)
    }
  }
    ...
    <div className="flex flex-col gap-2">
      <div className="flex items-center gap-2">
        ...
        >
          <UploadSimpleIcon
            weight="bold"
            className="size-4 text-[var(--ov-text-dim)]"
          />
          <span className="text-xs text-[var(--ov-text-dim)]">
            Drop a Hundred Rabbits .svg
          </span>
        </div>
