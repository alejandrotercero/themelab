...
import { formatComponentTrace, type TraceStackFrame } from "../component-trace.js";

const appFrame = (name: string, filePath: string, line = 10): TraceStackFrame => ({
  componentName: name,
  filePath,
  lineNumber: line,
  columnNumber: 5,
...
describe("formatComponentTrace", () => {
  it("renders an app frame with its location", () => {
    expect(formatComponentTrace([appFrame("Button", "src/Button.tsx", 12)])).toBe(
      "  in Button (at src/Button.tsx:12:5)",
    );
  });

  it("renders a dependency frame by name + package, never a path", () => {
    const out = formatComponentTrace([pkgFrame("Tabs", "@radix-ui/react-tabs")]);
  ...
  it("does not spend the line budget on a shared-UI frame", () => {
    ...
    expect(lines.some((l) => l.includes("src/components/ui/card.tsx"))).toBe(true);
  ...
  it("collapses consecutive frames from the same dependency", () => {
    ...
    const lines = formatComponentTrace(frames).split("\n");
    expect(lines.filter((l) => l.includes("@radix-ui/react-dialog"))).toHaveLength(1);
  });
