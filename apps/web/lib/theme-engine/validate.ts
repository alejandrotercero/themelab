...
import type { HrTheme, LuminanceLevel, LuminanceReport, NativeMode, Verdict } from "./types";
...
export function analyze(theme: HrTheme): LuminanceReport {
  const levels: LuminanceLevel[] = Object.entries(theme.slots)
    .map(([slot, hex]) => ({ slot: slot as LuminanceLevel["slot"], hex: hex as string, lStar: lStar(hex as string) }))
    .sort((a, b) => a.lStar - b.lStar);

  const uniqueValues = new Set(levels.map((l) => l.lStar.toFixed(1)));
  const uniqueCount = uniqueValues.size;
  const min = levels.length ? levels[0].lStar : 0;
  const max = levels.length ? levels[levels.length - 1].lStar : 0;
  ...
  if (uniqueCount >= MIN_UNIQUE_LEVELS && range >= MIN_RANGE) {
    verdict = "pass";
    notes.push(`${uniqueCount} distinct lightness levels spanning ${range} — rich enough for both modes.`);
    ...
    verdict = "fail";
    notes.push(`Just ${uniqueCount} distinct levels — too sparse to reconstruct a full design system.`);
  }

  if (range < MIN_RANGE && verdict !== "fail") {
    notes.push(`Lightness range ${range} is below ${MIN_RANGE}; extreme stops (50/950) are extrapolated.`);
  }
  ...
  return { levels, uniqueCount, min, max, range, nativeMode, verdict, score, notes };
}
