...
export function hslTriple(value: string): string {
  ...
  return `${r(h.h ?? 0)} ${r((h.s ?? 0) * 100)}% ${r((h.l ?? 0) * 100)}%`;
}
...
export function reformat(value: string, format: ColorFormat): string {
  ...
  if (format === "hex") return formatHex(parsed) ?? value;
  if (format === "hsl") return formatHsl(parsed);
...
export function oklchToHex(o: Oklch): string {
  return formatHex({ mode: "oklch", l: clamp(o.l, 0, 1), c: Math.max(0, o.c), h: o.h ?? 0 }) ?? "#000000";
}
...
export function sampleRamp(anchors: Oklch[], targetL01: number): Oklch {
  if (anchors.length === 0) return { mode: "oklch", l: targetL01, c: 0, h: 0 };
  const sorted = [...anchors].sort((a, b) => a.l - b.l);
  const t = clamp(targetL01, 0, 1);

  let base: Oklch;
  if (t <= sorted[0].l) {
    base = withL(sorted[0], t);
  } else if (t >= sorted[sorted.length - 1].l) {
    base = withL(sorted[sorted.length - 1], t);
  } else {
    let lo = sorted[0];
  ...
  const edge = t > 0.92 ? (1 - t) / 0.08 : t < 0.08 ? t / 0.08 : 1;
