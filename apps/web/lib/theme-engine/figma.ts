...
export function scalesToFigmaSvg(opts: FigmaSvgOptions): string {
  ...
  const neutralBrand = nHex[Math.min(Math.floor(count * 0.85), count - 1)] ?? nHex[count - 1] ?? "#111111";
  ...
  const alphas =
    count === 12
      ? [0.015, 0.035, 0.07, 0.11, 0.17, 0.26, 0.38, 0.52, 0.65, 0.76, 0.84, 0.92]
      : [0.02, 0.045, 0.085, 0.13, 0.20, 0.30, 0.42, 0.55, 0.68, 0.80, 0.91];
  ...
  // Build rect rows.
  const makeRow = (fills: string[], y: number, family: "primary" | "neutral", isAlpha: boolean) =>
    fills
      .map((fill, i) => {
