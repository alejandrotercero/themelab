// Ported verbatim from react-grab (packages/react-grab/src/utils/freeze-updates.ts).
//
// RISK: this patches React's INTERNAL dispatcher globally to pause rendering.
// It is OFF by default in ThemeLab (see SELECTION_UPDATE_FREEZE_ENABLED in
// selection.ts) and must stay off until validated across React 18/19, Next.js
// (Webpack + Turbopack) and Vite. Keep changes here in sync with upstream.
//
// During selection the page should freeze, but there is no public React
