...
function findStop(scale: ReturnType<typeof buildScale>, stop: number) {
...
describe("buildScale", () => {
  ...
  it("neutral scale has lower chroma at stop 500 than chromatic scale of same anchor", () => {
    ...
    const chromatic500 = findStop(chromatic, 500)
    const neutral500 = findStop(neutral, 500)

    const chromaticOklch = mustToOklch(chromatic500.value)
    const neutralOklch = mustToOklch(neutral500.value)
  ...
  it("neutral scale clamps chroma to ≤ 0.02 at stop 500", () => {
    ...
    const stop500 = findStop(neutral, 500)
    const oklch = mustToOklch(stop500.value)
